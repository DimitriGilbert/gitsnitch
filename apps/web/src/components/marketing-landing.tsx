...
const usageSteps = [
  {
    title: "Install",
    commands: ["npx @git-snitch/cli repo"],
  },
  {
    title: "Report for this repo",
    commands: ["npx @git-snitch/cli repo --open", "npx @git-snitch/cli repo --json > data.json"],
  },
  {
    title: "Scan across projects",
    commands: ["npx @git-snitch/cli scan . --period 14d --open --ai-usage", "npx @git-snitch/cli scan ~/workspace --period 3m --exclude \"legacy-*\""],
  },
  {
    title: "Share without leaking names",
      ...
      <div className="flex items-center justify-between rounded-full border border-white/15 bg-black/55 px-4 py-3 shadow-2xl shadow-black/40 backdrop-blur-xl md:px-6">
        ...
        <div className="flex items-center gap-2">
          <a href="/example.html" target="_blank" rel="noopener noreferrer" className={cn(buttonVariants({ variant: "outline", size: "sm" }), "gap-1.5 rounded-full border-white/25 bg-white/10 px-4 text-white hover:bg-white/20 hover:text-white")}>
            Repo Demo <ExternalLink className="size-3.5" />
          </a>
          <a href="/weekload.html" target="_blank" rel="noopener noreferrer" className={cn(buttonVariants({ variant: "outline", size: "sm" }), "gap-1.5 rounded-full border-white/25 bg-white/10 px-4 text-white hover:bg-white/20 hover:text-white")}>
            Scan Demo <ExternalLink className="size-3.5" />
          </a>
...
function Hero() {
  return (
    <section id="top" className="relative isolate flex min-h-svh items-center justify-center overflow-hidden px-5 py-24 md:py-32">
    ...
    <section id="features" className="px-5 py-16 md:py-24">
      <div className="mx-auto max-w-7xl">
        <div className="mb-16 gap-8">
          <h2 className="text-5xl font-semibold tracking-[-0.06em] text-white md:text-7xl">
            Reports shaped for local evidence, <br/>not cloud dashboards.
          </h2>
        ...
        <div className="grid grid-flow-dense auto-rows-auto gap-4 md:grid-cols-6">
          {features.map((feature) => {
            const Icon = feature.icon;

            return (
              <article key={feature.title} className={cn("group overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.055] p-6 shadow-2xl shadow-black/25 transition-colors hover:bg-white/[0.08]", feature.className)}>
...
function UsageGuide() {
  return (
    <section id="usage" className="px-5 py-16 md:py-24">
      <div className="mx-auto max-w-7xl">
        <div className="mb-16">
          <h2 className="max-w-4xl text-5xl font-semibold tracking-[-0.06em] text-white md:text-7xl">
            Scan a single repo,<br/>Or your whole workspace
          </h2>
        </div>
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-zinc-950 shadow-2xl shadow-black/40">
          <div className="space-y-0 p-0">
            {usageSteps.map((step, stepIndex) => (
              <div key={step.title} className={stepIndex > 0 ? "border-t border-white/[0.04]" : ""}>
                <div className="px-6 pt-5 pb-1 text-xs font-medium uppercase tracking-widest text-zinc-600">{step.title}</div>
...
function FinalAction() {
  return (
    <footer className="px-5 pb-10 pt-16 md:pt-24">
