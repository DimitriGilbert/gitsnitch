...
npm_publish() {
  ...
  if [[ "$DRY_RUN" == true ]]; then
    dryrun "pnpm -r ${filters[*]} publish --access public --no-git-checks"
    pnpm -r "${filters[@]}" publish --access public --no-git-checks --dry-run
    return 0
  fi

  info "Publishing ${#PUBLISHABLE[@]} packages (topological order)..."
